...
interface InterpretacaoFinal {
  avaliacao_ia: string;
  confiabilidade_modelo: number;
  nota_global: number;
}
 ...
 */
function analisarImagensTricoscopicas(
  imageUrls: string[],
  clienteId: string,
  avaliacaoId: string,
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { avaliacao_id, image_urls, cliente_id }: AnaliseRequest = await req.json()

    if (!avaliacao_id || !image_urls || !cliente_id) {
      return new Response(
    ...
    const { data: avaliacaoAnterior } = await supabaseClient
      .from('avaliacoes')
      .select('*')
